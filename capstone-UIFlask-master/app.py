...
def index():
    ...
    Total_rating = pd.crosstab(
        index= df2['Content Rating'], 
        columns='Total Rating'
    ).sort_values(by='Total Rating',ascending=False)
    X = Total_rating.index
    Y = Total_rating['Total Rating']
    ...
    plt.barh(X,Y, color=my_colors)
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('Total_rating.png',bbox_inches="tight") 
